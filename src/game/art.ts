...
export function createCannon(scene: Phaser.Scene): Phaser.GameObjects.Container {
  ...
  // Shadow under base
  const shadow = scene.add.ellipse(0, 30, 82, 24, 0x000000, 0.32);

  // Base plate — darker with highlight edge
  const base = scene.add.rectangle(0, 12, 76, 38, 0x1e2f3e);
  base.setStrokeStyle(3, 0x5a8aaa, 0.8);

  // Turret body — medium blue-gray
  const turret = scene.add.circle(0, -2, 24, 0x2c4560);
  turret.setStrokeStyle(3, 0x7ab0cc, 0.7);

  // Barrel — art points UP by default, will be rotated
  const barrel = scene.add.rectangle(0, -42, 15, 54, 0x1e3040);
  barrel.setStrokeStyle(2, 0x7ab0cc, 0.85);

  // Barrel tip / muzzle ring — wider and darker
  const muzzle = scene.add.rectangle(0, -71, 20, 14, 0x152030);
  muzzle.setStrokeStyle(2, 0xaacce0, 0.9);

  // Highlight stripe along barrel
  const highlight = scene.add.rectangle(-5, -38, 5, 38, 0xffffff, 0.18);

  // Turret dome highlight
  const domeHi = scene.add.circle(-5, -8, 7, 0xffffff, 0.12);

  container.add([shadow, base, turret, barrel, muzzle, highlight, domeHi]);
